...
claude-agent-acp  codex-acp    gemini --acp   copilot --acp   … any ACP agent
   ▲                ▲               ▲               ▲
   └────────────────┴───────┬───────┴───────────────┘
                            │ opted-in + trusted only (ADR-0014)
                    ┌───────┴────────┐
...
**Connect.** The wizard is the only thing that records Runtime Trust, so nothing else can start an agent. The window's own trust gates it first, because probing starts an agent like anything else. Then it resolves every runtime, asks which one, offers to install a missing adapter at the exact version the catalog names — otherwise the reason shown against a runtime is advice the wizard cannot act on — and asks what to launch when a command is still missing. Where there is no adapter to offer, the catalog's own record of how that vendor documents installing it is shown instead: text to copy, never a command run for the user, since installing a CLI is theirs to do (ADR-0007). What a CLI needs *after* installing before it will speak ACP — DeepSeek Harness's profile is the one today — is shown there and again beside a session that would not open, which is where a CLI that is present but unconfigured actually fails.
...
The wizard records no suppression evidence — neither a tool list, because ACP v1 has no call that returns one, nor the consent a plan that edits a workspace file needs. Nothing waits on it: the Shim is injected on opt-in and trust, and a CLI's own subagents are allowed beside it (ADR-0014). What the wizard does record is Fan-out Consent, for a Runtime on another provider.
...
- Orchestration is off by default: no Session Capability is issued, no Shim is injected, and no spawn RPC exists while it is off. On, the Shim goes to every trusted runtime whose agent accepts MCP servers, below the Depth Cap. A CLI's own subagents are allowed beside it (ADR-0014): they run inside the same session, so the adapter is designed to route their permission requests through it like the parent's, and their cost is its cost — but that routing is the vendor's to keep working, not something this client can verify; they are not counted against the limits, which bound only what goes through the Shim; and what the harness decides without asking this client never sees.
