...
Turn orchestration on and the agent you are talking to can hand work to agents on other CLIs — a different CLI, model or effort per task — within limits you set: how deep, how many at once, how many in all, how long. A CLI may also fork its own helpers inside its session to split a task further; those are the CLI's own, under its own permission mode, and outside your subagent limits. `docs/CHANGELOG.md` records what was verified and when.
...
- **Handing work to other CLIs is off until you turn it on.** While it is off, no delegation server is injected into any session. On, every subagent is bounded by your limits, and handing work to a CLI on another provider needs your explicit consent first.
...
- **Claude uses the login Claude Code already has.** Sign in once with `claude /login` and sessions run on your plan, the way Zed's do. An organisation that wants an API key enforced sets `agentConductor.claude.hideSubscriptionAuth`, which launches the adapter with `--hide-claude-auth` — part of what you approve, either way. Keys live in VS Code's secret storage; your settings hold only the name of the key, never its value.
