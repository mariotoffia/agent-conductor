...
| **Session Capability** | Short-lived authority for the Shim, bound on our side to one active parent Session, a depth, a set of roots, an expiry, a list of allowed methods, and current Runtime Trust. The Shim presents it as a **secret**, which reaches it through its environment (`AGENT_CONDUCTOR_SESSION_SECRET`) and never its arguments. |
