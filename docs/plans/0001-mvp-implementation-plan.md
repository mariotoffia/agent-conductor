...
Status: complete — every task done, gates evidenced 2026-08-24. Plans are temporary; the durable decisions live in the ADRs and root documents, and the per-release verification record in `docs/CHANGELOG.md`. Delete this file once nobody needs the history in it.

## Milestone Status

Marked from the fresh 2026-08-24 run of `make check-all`: 857 unit tests and 15 extension-host tests, all passing, plus `make package` producing one extension file.
...
| Stable direct Session | `@conductor` streams messages, thoughts, tool calls, diffs, permissions, usage, and Read-back | Done — driven end to end in the extension host against the mock Agent |
| Wizard, settings, and multiple Runtimes | Claude, Codex, Gemini, and Copilot can be validated and configured from live Config Options | Done against the mock Agent — detect, trust, probe, discovery, smoke test and save all pass; live-CLI validation stays manual by design (§Deferred Work) |
| Orchestrator | Suppression, authenticated Shim tools, worktrees, limits, cancellation cascade, and Sessions tree pass integration tests | Done in tests — the delegation suite passes end to end with a test-only tool list; in the product no Runtime can hold a Suppression Capability, so the Shim is injected for nobody (ADR-0008) |
| Real release gate | `make check-all` runs non-empty extension-host tests against the mock Agent | Done — the reporter refuses a run with no count, and the fresh run counted 15 |
...
| ACP-agent Facade | Conductor is exposed as an ACP Agent by reusing `src/core` | Deferred — to be planned separately now that the MVP gates pass |
...
The boxes below were never marked, though the work landed long ago; marked now from what is in the tree today: ADRs 0007–0010 all read `Status: accepted`, the manifest defaults `orchestration.enabled` to `false` and carries `secretEnvironment` with both pinned by `manifest_security.test.ts`, every listed term has its `UBIQUITOUS.md` entry, and the gates are green in this plan's closing run.

- [x] Draft the four ADRs with `Status: proposed`, alternatives, consequences, and the exact trust, lifecycle, worktree, provider, authentication, and secret decisions listed above.
- [x] Obtain repository-owner approval for all four ADRs and change their status to `accepted`; do not begin Agent process implementation while any remains proposed.
- [x] Change orchestration to explicit opt-in in `package.json`:

```json
"agentConductor.orchestration.enabled": { "type": "boolean", "default": false }
```

- [x] Replace plaintext Runtime `env` with secret references:
...
- [x] Define `RuntimeTrust`, `SuppressionCapability`, `BudgetCapability`, `SessionCapability`, and versioned `PersistedSession` in the canonical docs before adding identifiers to code.
- [x] Run `make lint && make test`; expect exit 0. Commit: `docs: ratify agent trust and session lifecycle`.
...
- [x] Run focused wizard tests and the mock-Agent smoke integration test; expect pass. Commit: `feat: connect and validate ACP runtimes`.
  ...
  - From the working tree's own judged audit of this plan (deleted with the other temporary guidance), the issues it raises against implemented code: a credential pasted into a Suppression Plan's environment now meets the same tripwire `secretEnvironment` values do; dismissing the fan-out notice cancels the run instead of quietly declining, and declining is now a choice; recording the approval sits inside the same guard as the settings write; the offline wording of a failed Registry refresh is tested, which moved `describeRefresh` into the core where it can be; scratch files a crashed host left under global storage are cleared by the next write; the real filesystem port has direct tests for symlinks, directories and the digest ceiling. Two more it raised were already closed by the work above.
  ...
  - Pre-existing, from the working tree's own judged audit, against code that is already built: the dialog that approves a command an Agent wants to run listed the variables the Agent set and said nothing about the rest — the command also inherits this window's whole environment, which on the machine somebody develops on is where their credentials are. It says so now, above the list and paid for out of the same budget, and a test pins that the parts still add up to less than the dialog will show, since a sentence added without being paid for is how they stop adding up. An Agent reporting that it is running a model its own list does not contain is read back as what it said rather than as the nearest listed thing, which is now pinned. A `config_option_update` arriving before setup has answered with a session id is nobody's, and is no longer adopted. Which settings the Client parses but nothing acts on is a test now, not a paragraph here. Left alone deliberately: the case-spelling ceilings are documented and fail in the safe direction, and re-verifying every Runtime's suppression recipe against its vendor's own documentation needs sources this branch cannot reach. `src/test/mock-agent.ts` is within thirty lines of the size limit and grows with every scenario added to it; it needs splitting along the same seam as its options — the prompt behaviours — before the next one is written.
...
**Files:** Modified `README.md`, `walkthrough/orchestrate.md`, `package.json`, `AGENTS.md`, `ARCHITECTURE.md`, ADR-0004's references, this plan; created `docs/CHANGELOG.md` and `src/test/unit/file_length.test.ts`; deleted the temporary implementation guide and the judged audit.

- [x] Remove bootstrap usability claims until the direct vertical slice passes; then document actual prerequisites, trust, auth, cost, provider egress, worktree limits, and supported Runtime capabilities.
  - The slice passed, so the second half applied. What still outran the code was fan-out: the README's headline, the Marketplace description and the orchestration walkthrough all offered cross-CLI subagents, while ADR-0008's consequence is that no Runtime can hold a Suppression Capability today. The README, `AGENTS.md` and the walkthrough now say the machinery is built and tested, that it is off for every CLI — not merely off by default — and why; the Marketplace description simply no longer offers it. The walkthrough gained the provider-egress sentence: a Brief is words and paths, but the agent on the other end reads files itself, and what it reads goes to its provider.
  - A settings description offered `/spawn`, a command the participant has never declared; presets are the Orchestrator's subagent defaults and the description now says that and no more. Trust, authentication, cost-or-unknown and worktree limits were already documented truthfully by the earlier tasks and are untouched.
- [x] Mark milestones from executable evidence only. Promote remaining durable guidance from temporary plans into canonical docs/ADRs, then delete obsolete plan content without leaving code/test references to it.
  - The milestone table above is written from the fresh gate run, and says so. Promoted before deletion: the per-CLI verification into `docs/CHANGELOG.md`, and the render map's home into `ARCHITECTURE.md §Data flows` — the map itself is `src/vscode/render.ts`, whose compile-time proof covers every Update variant. The suppression recipes and their traps already lived in `src/core/policy.ts` beside golden fixtures. `AGENTS.md` and ADR-0004 now point at the changelog where they pointed at the guide's appendix; grep shows no code, test or durable doc naming either deleted file.
- [x] Verify current ACP and vendor claims against primary sources before release; record verification date and links in release documentation, not code.
  - `docs/CHANGELOG.md` holds the record, dated 2026-08-24: the ACP schema's missing tool-list call (the fact ADR-0008 rests on), both adapter packages and their pinned versions with their `bin` names, the Registry feed, `gemini --acp`, Copilot's per-session `--acp --stdio` flags, and Anthropic's third-party authentication policy re-checked as ADR-0010 requires. The suppression recipes' internal tool names remain unverified and the record says so — nothing rests on them while suppression is fail-closed.
  - Dated verification earned its keep at once: an earlier audit had "confirmed against the npm registry" that the Claude adapter's bin is `claude-code-acp`; version 0.70.0's own metadata says `claude-agent-acp`, which is what the catalog launches. Two contradictory confirmed claims, distinguished only by their dates.
- [x] Run `make lint` and `make test`; expect pass. Run `make check-all`; expect a real integration count greater than zero.
  - Fresh on 2026-08-24: lint green, 857 unit tests passing, 15 extension-host tests passing. The delegation suite — the flake ADR-0012 was written for — held.
- [x] Confirm all code files are at most 500 lines and docs at most 600 lines; split by responsibility when exceeded.
  - The confirmation is executable now: `src/test/unit/file_length.test.ts` sweeps every git-tracked code file against 500 and every document against 600, counts the way `wc -l` counts, and carries a self-test proving the sweep can fail — the shape of claim an earlier round found being made against two directories rather than the repository. Nothing needed splitting; three files sit exactly at the code cap.
