...
async function workspace(t: TestContext): Promise<string> {
  const made = await mkdtemp(join(tmpdir(), "conductor-fs-"));
...
test("an open buffer is served ahead of what is on disk", async (t: TestContext) => {
...
test("a file nobody has open is read from disk", async (t: TestContext) => {
  const root = await workspace(t);
...
test("line and limit window the content the agent gets", async (t: TestContext) => {
  const root = await workspace(t);
...
test("a path outside the session roots is refused, not offered for consent", async (t: TestContext) => {
  const root = await workspace(t);
...
test("a symlink pointing out of the roots does not smuggle a path back in", async (t: TestContext) => {
  const root = await workspace(t);
  const outside = await workspace(t);
  await writeFile(join(outside, "secrets.env"), "TOKEN=1\n");
...
test("a relative path is refused: ACP paths are absolute", async (t: TestContext) => {
  const root = await workspace(t);
  const fs = new WorkspaceFs({ roots: [root], documents: buffers(), consent: consent(true) });
...
test("a refused operation reads nothing", async (t: TestContext) => {
  const root = await workspace(t);
  await writeFile(join(root, "a.ts"), "on disk\n");
...
test("writing an open file goes through the editor, not around it", async (t: TestContext) => {
  const root = await workspace(t);
  const file = join(root, "a.ts");
  await writeFile(file, "on disk\n");
  const documents = buffers({ [file]: "unsaved edit\n" });
...
test("writing a closed file creates it, and its directory, on disk", async (t: TestContext) => {
  const root = await workspace(t);
...
test("a write outside the roots never reaches the disk", async (t: TestContext) => {
  const root = await workspace(t);
  const outside = await workspace(t);
  const fs = new WorkspaceFs({ roots: [root], documents: buffers(), consent: consent(true) });
...
test("an additional root is a root too", async (t: TestContext) => {
  const root = await workspace(t);
  const extra = await workspace(t);
...
test("a buffer the editor knows under an uncanonical path is still served first", async (t: TestContext) => {
  const root = await workspace(t);
  const file = join(root, "a.ts");
  await writeFile(file, "on disk\n");
  // What the editor reports for a workspace reached through a symlink.
  const linkedRoot = join(await workspace(t), "link");
...
test("consent is asked for under the operation the acp method authorizes", async (t: TestContext) => {
  const root = await workspace(t);
  await writeFile(join(root, "a.ts"), "on disk\n");
  const guard = consent(true);
  const fs = new WorkspaceFs({ roots: [root], documents: buffers(), consent: guard });
